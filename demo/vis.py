...
def show2Dpose(kps, img):
    ...
    for j, c in enumerate(connections):
        ...
        cv2.line(img, (start[0], start[1]), (end[0], end[1]),
                 lcolor if LR[j] else rcolor, thickness)
        cv2.circle(img, (start[0], start[1]),
                   thickness=-1, color=(0, 255, 0), radius=5)
        cv2.circle(img, (end[0], end[1]), thickness=-
                   1, color=(0, 255, 0), radius=5)
...
def img2video(video_path, output_dir):
    ...
    videoWrite = cv2.VideoWriter(
        output_dir + video_name + '.mp4', fourcc, fps, size)
...
def get_pose3D(video_path, output_dir):
    ...
    model_path = "/home/jeff/Strided-PostureReader/demo/lib/checkpoint/no_refine_4365.pth"
    ...
    # input
    keypoints = np.load(output_dir + 'input_2D/keypoints.npz',
                        allow_pickle=True)['reconstruction']
    ...
    for i in tqdm(range(video_length)):
        ...
        input_2D_aug[:, :, 0] *= -1
        input_2D_aug[:, joints_left +
                     joints_right] = input_2D_aug[:, joints_right + joints_left]
        ...
        plt.savefig(output_dir_3D + str(('%04d' % i)) + '_3D.png',
    ...
    for i in tqdm(range(len(image_2d_dir))):
        ...
        edge = 130
        image_3d = image_3d[edge:image_3d.shape[0] -
                            edge, edge:image_3d.shape[1] - edge]
        ...
        plt.savefig(output_dir_pose + str(('%04d' % i)) +
                    '_pose.png', dpi=200, bbox_inches='tight')


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--video', type=str,
                        default='sample_video.mp4', help='input video')
