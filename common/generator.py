...
class ChunkedGenerator:
    def __init__(self, batch_size, cameras, poses_3d, poses_2d,
        ...
        if poses_3d is not None:
            self.batch_3d = np.empty(
                (batch_size, chunk_length, poses_3d[key].shape[-2], poses_3d[key].shape[-1]))
        self.batch_2d = np.empty(
            (batch_size, chunk_length + 2 * pad, poses_2d[key].shape[-2], poses_2d[key].shape[-1]))
    ...
    def get_batch(self, seq_i, start_3d, end_3d, flip, reverse):
        ...
        if pad_left_2d != 0 or pad_right_2d != 0:
            self.batch_2d = np.pad(seq_2d[low_2d:high_2d], ((
